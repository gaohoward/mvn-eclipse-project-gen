...
import hornetq_project_gen.wildfly.WildFlyVersionedModelHelperFactory;
...
public class WildFlyImportWizardPage extends WizardPage {

    protected WildFlyImportWizardPage(String pageName, IStructuredSelection selection) {
        super(pageName);
        ...
        editor.getTextControl(fileSelectionArea).addModifyListener(
                new ModifyListener() {
                    public void modifyText(ModifyEvent e) {
                        File sourceRoot = new File(WildFlyImportWizardPage.this.editor
                                .getStringValue());
                        File pom = new File(sourceRoot, "pom.xml");

                        if (pom.exists()) {
                            try {
                                helper = WildFlyVersionedModelHelperFactory
                                        .createModelHelper(sourceRoot, pom);
                                Model model = helper.getModel();
                                if (!"org.wildfly".equals(model.getGroupId())
                                        || !"wildfly-parent".equals(model
                                                .getArtifactId())) {
                                    setErrorMessage("Not a WildFly source root!");
                                }
